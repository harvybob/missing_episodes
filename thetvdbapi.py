...
class TheTVDB(object):
    ...
    def get_episode_by_season_and_episode_number(self, series_id, season_number, episode_number):
        """Get the episode object matching this series id and season/episode numbers."""
        
        url = "%s/series/%s/default/%s/%s" % (self.base_key_url, series_id, season_number,episode_number)
        data = urllib.urlopen(url)
    ...
    def get_episode_by_air_date(self, series_id, date):
		url = "%s/GetEpisodeByAirDate.php?apikey=%s&seriesid=%s&airdate=%s" % (self.base_url, self.api_key, series_id, date)
		data = urllib.urlopen(url)
		print url
		episode = None
		try:
